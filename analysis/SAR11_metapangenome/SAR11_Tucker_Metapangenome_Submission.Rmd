...
First we need to assemble our set of high quality genomes.
Take all the isolate genomes from Pelagibacteraceae published in Freel et al., 2024 and those that are previously published and select only those that are high quality. Use checkM to make sure that you have the most highly complete genomes. We are using a threshold of >95% completion, <5 contamination, and fewer than 50 contigs.

Note that the code here is written using historical subgroup names. You can see how those correspond to new genus names in Supplemental file 1a (https://doi.org/10.6084/m9.figshare.28087925.v2)

```{python}

for i in *.fa; do
  anvi-script-reformat-fasta $i -o "fixed-fasta/$i" -l 0 --simplify-names;
done
...
##run checkM
mkdir checkm
checkm lineage_wf -t 40 -x fa updated/ checkm/

#remove genomes that don't make the threshold of >95% completion, <5 contamination, and fewer than 50 contigs.
```



Let's build a tree with outgroups, SAR11 isolates from other families, and all the high quality genomes from Pelagibacteraceae to see where they fall. We will use the Alphaproteobacteria HMM (named SAR11_165) from Freel et al., 2024 to get the set of genes we will build the tree from. 
```{bash}
#Move the Alphaproteobaccteria HMM into working directory
#make sure you have all quality controlled Pelagibacteraceae genomes, SAR11 isolates from other families, and outgroups
...
#can use a random subset of the metagenomes to build the profile db for the tree (the actual read recruitment will be conducted on just a Pelagibacteraceae contig db in the next step) (Metagenomes and their accession IDs are available in Supplemental File 1c) (https://doi.org/10.6084/m9.figshare.28087925.v2))
...
#use a small set of metagenome samples to build the profiles on (not necessary to run on full data)
for sample in `awk '{print $1}' samples_test.txt`
do
    if [ "$sample" == "sample" ]; then continue; fi
...
##Now that we have a nice tree with outgroups, let's focus on the metapangenomic analysis of just the isolate genomes from Pelagibacteraceae. 
####the main contig db
```{r}
#move just the pelagibacteraceae genomes into one folder and concatenate those files into one file 

for files in *.fa 
do 
cat ${files} >> SAR11_Oct2024_isolates.fa 
done
...
############################################################
###Characterisitics 
Using output from the summaries created in anvio and from checkM (for completion and contamination) lets describe the genomes. 
```{r}
...
#output from checkM can be added in (Will need to reformat their output file)
checkm=read.delim("checkm_final_genomes_organized.txt")
names(checkm)
checkm=checkm%>%dplyr::rename("layers"="genome")
...
colors_tog=c("Ia.1.I" ="#808000", "Ia.3.I" ="#eee8aa" , "Ia.3.II"  = "#3eafe0",  "Ia.3.III"="#adff2f"  ,  "Ia.3.IV"="#800000", "Ia.3.V"="#ba55d3","Ia.3.VI"="#0000cd","Ia.4.II"="#ff8c00" , "Ia.4.RS39"="#ffd700", "Ib.1"="#dda0dd", "Ia.4.1483"="#F91EAE","previous" ="#9FE2BF", "new" ="#4682B4" , "other_locations"  = "#ccdeea")



colorsgenus<- c("Ia.1.I" ="#808000", "Ia.3.I" ="#eee8aa" , "Ia.3.II"  = "#3eafe0",  "Ia.3.III"="#adff2f"  ,  "Ia.3.IV"="#800000", "Ia.3.V"="#ba55d3","Ia.3.VI"="#0000cd","Ia.4.II"="#ff8c00" , "Ia.4.RS39"="#ffd700", "Ib.1"="#dda0dd", "Ia.4.1483"="#ff1493") 
...
test_tree
write.tree(tree3, "Pelagibacteraceae_tree_dropped_othersubclades_outgroup.newick") 
...
Quick look at the gANI (more analyses on this later)
...
##for Pelagibacteraceae as a whole
min_ani_Pelagi=long_ani3%>%dplyr::summarise(ani_min=round(min(value, na.rm = TRUE)*100,1), ani_mean=round(mean(value, na.rm = TRUE)*100,1),ani_sd=round(sd(value, na.rm = TRUE)*100,1), n=n())
...
##Lets characterize the total number of gene clusters, number of singletons, and size of core of the individual pangenomes of each genus (these data went into Supplemental file 2a); Note this will also help us define the gene clusters we will use in read recruitment analyses 
...
##the next section of code will accomplish a few things. We will characterize the different aspects of the pangenome, including genus-specific gene clusters, multi-genus specific gene clusters, and gene clusters that are found at least once in all 11 genera of Pelagibacteraceae. At the end we will have a table that we can then import into anvio to create Fig 2a. This will be repeated for the functional pangenome. 

##Lets characterize the genus-specific GCs. Later in the code we will show that Pelagibacteracea genera have coastal or offshore habitat distributions in the tropical Pacific or are rarely detected in the tropical Pacific. We can then assign these genus-specific gene clusters  to coastal or offshore (and rare) categories based on the environmental preference of the genomes in which they occurred.


```{r}
##first let us add all the genus specific GCs to the big summary of the pangenome
...
#write.table(pangenome_SAR11,"output/by_contigs_all_bins_Oct_Gene_Cluster_Summary_Tax.txt", row.names=FALSE, sep = "\t",quote = FALSE)
#pangenome_SAR11=read.delim("output/by_contigs_all_bins_Oct_Gene_Cluster_Summary_Tax.txt")
...
###uniquify the matrix to just have one row per gene cluster id 
...
##change absent genes from NA to absent
compare_cores4[is.na(compare_cores4)] <- "absent"
names(compare_cores4)

##output genus specific genes 
...
##Later in the code we will show that Pelagibacteracea genera have coastal or offshore habitat distributions in the tropical Pacific or are rarely detected in the tropical Pacific. We can assign these genus-specific gene clusters  to coastal or offshore (and rare) categories based on the environmental preference of the genomes in which they occurred.
...
##Next we will characterize gene clusters that are multi-genus specific (shared among all representatives of two or more genera and not found among all others). Similarly to the genus-specific clusters, we will also assign the multi-genus specific gene clusters to coastal or offshore (and rare) categories based on the environmental preference of the genomes in which they occurred.
```{r}
#get the gene clusters that are either absent or core (no intermediate distributions in the genera)
...
##remove genes clusters that are Core Pelagibacteraceae or Singletons 
...
##add info on functional annotation from the big gene cluster summary table to the multiple-genus specific table
...
##Later in the code we will show that Pelagibacteracea genera have coastal or offshore habitat distributions in the tropical Pacific or are rarely detected in the tropical Pacific. We can assign these multi-genus specific gene clusters  to coastal or offshore (and rare) categories based on the environmental preference of the genomes in which they occurred. 
...
###We will use numeric calculations to assess the sets of multi-genus gene clusters that are only shared among genera that have coastal, offshore, or rare distributions in the tropical Pacific 
...
##find out which are only found in rare genera
...
##find out which are only found offshore (note if the gene clusters shows up in rare genera, it can still count toward offshore, but if it show up in coastal, it cannot )
...
##find out which are only found coastal (note if the gene clusters shows up in rare genera, it can still count toward offshore, but if it show up in coastal, it cannot )
...
##add this info all to one data frame
...
##add this dataframe of multi-genus specific gene clusters to the large data frame and clean it up 
...
table(strict_multilineage_core_not_genus$Enviro_Specific)


##check your genus-specific gene clusters with ComplexUpset, this also shows you that multi-genus specific genes were generally uniformly distributed across many different groupings of genera
...
### We next wanted to characterize gene clusters that were shared by all 11 genera. This include intermediate distributions, meaning these genes clusters are not necessary core across all 11 genera, but counted if a gene cluster is detected at least once in a given genus 

```{r}

#have the list of genus-specific and multi-genus gene clusters handy 
listy=strict_multilineage_core$gene_cluster_id
listy
length(unique(listy))
182+206
#strict multilineage + genus-specific genes 
...
gc_id=test
dim(gc_id)
8242-784-4201-182-206
#here we have total pangenome size - core Pelagibacteraceae- singleton pelagibacteraceae-strict multilineage - genus-specific genes 
#2869

##make this binary, this time allowing for intermediate distributions
...
##make it into a useable table with info on annotation
...
write.csv(tog, "output/Sup_File_2a_Conventional_pangenome_info.csv")
...
###
Add this information to the conventional pangenome to create Fig 2a. 
###
...
##this is a table of the genus specific gene clusters characterized above. 
...
####
Let's create a functional pangenome of Pelagibacteraceae genomes. This is focused on genes with well-characterized roles (functions) in known metabolisms by examining those that share orthology with genes described in the Kyoto Encyclopedia of Genes and Genomes (KEGG; Kanehisa and Goto 2000) through KOfam models (Aramaki et al. 2020) and generating a ‘functional pangenome’ (Fig. 2b). 
####
...
####
##the next section of code will accomplish a few things. We will characterize the different aspects of the functional pangenome, including genus-specific functions, multi-genus specific functions, and functions that are found at least once in all 11 genera of Pelagibacteraceae. At the end we will have a table that we can then import into anvio to create Fig 2b. 


Let's characterize some basics 
...
##from this we gather that ther are 612 KOfams core to all 92 genomes, and 70 singletons, 1276 total func
...
```{r}
##to get the ids of those groups
...
##similar to the conventional pangenome we will need to identify genus-specific functions and then assign those to coastal and offshore (and rare) categories based on their distribution in genomes with those habitat preferences 
...
```{r}
##first let us add all the genus specific functions to the big summary of the functional pangenome
...
###uniquify the matrix to just have one row per function
...
##change absent functions from NA to absent
compare_cores4[is.na(compare_cores4)] <- "absent"
names(compare_cores4)

##output genus specific functions
#specific Ib.1
test=compare_cores4%>%dplyr::filter(bin_Ib.1 == "Core" & bin_Ia.3.VI == "absent" & bin_Ia.3.V == "absent" & bin_Ia.3.IV == "absent" & bin_Ia.3.III == "absent" & bin_Ia.3.II == "absent" & bin_Ia.3.I == "absent" & bin_Ia.1.I == "absent" & bin_Ia.4.II == "absent" & bin_Ia.4.RS39 == "absent" & bin_Ia.4.1483== "absent")
#5 unique Ib.1 genes
...
###All functions- HIMB1483 is not included in this because, come on, its only one genome 
...
#specific Ia.3.VI, Ia.3.II. Ia.3.III (coastal)
...
coastal_specific_func=left_join(test, KOfam_add_on, by="Func_Cluster_id")
##this is the fibronectin adhesion gene we get to later on :P
...
###next we characterize multi-genus specific function
...
##note this will remove HIMB1483 (Ia.4.1483) genus-specific genes
...
##check your genus-specific functions with ComplexUpset, this also shows you that multi-genus specific functions were generally uniformly distributed across many different groupings of genera, note we have left core pelagibacteraceae in this version of the upset plot 
library(ComplexUpset)
library(patchwork)
Genera = colnames(compare_cores11)
compare_cores11[Genera] = compare_cores11[Genera] == 1
...
##To characterize multi-genus specific functions, first remove core functions and then remove genus specific fucntions
...
###We will use numeric calculations to assess the sets of multi-genus specific functions that are only shared among genera that have coastal, offshore, or rare distributions in the tropical Pacific 
####
...
##find out which are only found offshore (note if the function shows up in rare genera, it can still count toward offshore, but if it shows up in coastal, it cannot )
...
##find out which are only found coastal(note if the function shows up in rare genera, it can still count toward coastal, but if it shows up in offshore, it cannot )
...
### We next wanted to characterize functions that were shared by all 11 genera. This includes intermediate distributions, meaning that if a function is detected at least once in a genus it will be marked present in that genus 
...
#quick complex upset to take a peak 
library(ComplexUpset)
library(patchwork)
...
###136 functions are distributed across all 11 genera 

```


Bring all the different facets of the functional pangenome together into one table to import in anvio for Fig 2b
```{r}
...
###
Add this information to the functional pangenome in anvio 
###
```{bash}
...
##in this next section we will look at read recruitment
To avoid biased estimates of coverage that can occur due to highly recruiting accessory genes, read recruitment was analyzed using only single-copy genes core to each genus. Those single-copy genes were identified previously in our analysis of the conventional pangenome. 
...
##Note this section can be conducted to set up the directory a new, but all non-outlier coverages are organized per genus and provided in the folder SAR11_Read_Recruitment_By_Genus/ on github
```{bash}
...
write.csv(SumsIa.3.V_v2, "output/SumsIa.3.V_v2.csv")
...
write.csv(SumsIa.3.VI_v2, "output/SumsIa.3.VI_v2.csv")
...
write.csv(SumsIa.3.I_v2, "output/SumsIa.3.I_v2.csv")
...
write.csv(SumsIa.3.II_v2, "output/SumsIa.3.II_v2.csv")
...
write.csv(SumsIa.3.III_v2, "output/SumsIa.3.III_v2.csv")
...
write.csv(SumsIa.3.IV_v2, "output/SumsIa.3.IV_v2.csv")
...
write.csv(SumsIa.4.II_v2, "output/SumsIa.4.II_v2.csv")
...
write.csv(SumsIa.4.RS39_v2, "output/SumsIa.4.RS39_v2.csv")
...
write.csv(SumsIa.1.I_v2, "output/SumsIa.1.I_v2.csv")
...
write.csv(SumsIb.1_v2, "output/SumsIb.1_v2.csv")
...
write.csv(SumsIa.4.1483_v2, "output/SumsIa.4.1483_v2.csv")


```


Before we bring all the genus-level read recruitment data together, correct for detection. A detection value of at least 0.25 was used as criteria to eliminate false positives that could arise if an isolate genome was falsely found within a sample (Utter et al. 2020).  
...
#write.table(freqs3,"output/SAR11_KByT_aloha_relative_read_recruit_corebygenera_normalized_oct.txt", row.names=TRUE, sep = "\t",quote = FALSE)
...
df1 <- tt %>%dplyr::arrange(factor(Metagenome_ID, levels = neworder))
df1
...
mini_enviro=Enviro%>%dplyr::select(Metagenome_ID, PRO.mL)
df1=left_join(df1, mini_enviro, by="Metagenome_ID")
write.csv(df1, "output/normalized_SAR11_genus_read_recruit_supptable_v2.csv")
``````




#### Let's understand the environmental conditions of our metagenomes. Metagenomes were taken from the Kane'ohe Bay Time-series and Hawaii Ocean Time-series. To increase the sample size, we an use all samples from these programs that have a full suite of environmental data ( Prochlorococcus cells per mL (PRO.mL), Synechococcus cells per mL (SYN.mL), Photosynthetic picoeukaryotes (PEUK.mL), Heterotrophic bacteria (HBACT.mL), chlorophyll a concentrations (chla.ug.per.L), silicate concentrations (Silicate), pH, Seawater temp (SW.Temperature.at.site), and Salinity) in addition to samples that also have metagenomes associated to them.  K-means clustering analysis of a scaled matrix of biogeochemical conditions was used to characterize the environmental background from which the metagenomic data were derived. 
...
#####Let's map the sites were the majority of the metagenomes come from and where the majority of the Pelagibacteraceae cultures were isolated from 
...
hawaii  <- ggplot(data = map) +
     geom_sf(fill = "grey") +
     geom_sf(data = sites, size = 1, shape = 24, fill = "#7d7d7d")+
     ...
     theme(panel.grid.major = element_blank(), panel.background = element_rect(fill = "#FFFFFF"), 
         panel.border = element_rect(fill = NA))+scale_x_discrete(position = "top")+ annotation_scale()
hawaii
ggsave("output/Hawaii_Islands_Oahu_scale.svg", width=2.5, height=2.5 )
...
hawaii_zoom  <- ggplot(data = map) +
     ...
     theme(panel.grid.major = element_blank(), panel.background = element_rect(fill = "#FFFFFF"), 
         panel.border = element_rect(fill = NA))+theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1))+ annotation_scale()
hawaii_zoom
ggsave("hawaii_zoom.svg", width=3, height=2)
...
#####overlay the distribution of Prochlorococcus cell per mL on the map
...
hawaii_zoom  <- ggplot() + geom_raster(data = ctd_mba, aes(fill = PRO.mL, y = latitude, x =longitude))+
 ...
 geom_point(data = south2, aes( x = longitude,y =latitude),
     ...
     theme(panel.grid.major = element_blank(), panel.background = element_rect(fill = "#FFFFFF"), 
         panel.border = element_rect(fill = NA))+theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1))+ annotation_scale()
hawaii_zoom
ggsave("output/color_raster_map_KByT_PRO_scaled_wmarker.svg", height=5, width=4)


```
#####plot Prochlorococcus cells per mL across metagenome samples 
...
##So now  we have the read recruitment for each genus per samples based on core read recruitment and standardized based on detection values, we can plot the read recruitment data of the Pelagibacteraceae genera

```{r}
#this file was created before 
#freqs_norare=read.csv("output/SAR11_KByT_aloha_relative_read_recruit_corebygenera_normalized_oct_norare.csv", row.names = 1)
...
test$rel_more_coastal_occurrences=""
test$rel_more_coastal_occurrences[test$bins=="HIMB83"]="Yes"
test$rel_more_coastal_occurrences[test$bins=="FZCC0015"]="Yes"
test$rel_more_coastal_occurrences[test$bins=="HIMB1456"]="Yes"
test$rel_more_coastal_occurrences[test$bins=="HIMB2250"]="Yes"
...
level_order <- factor(read_recruit$Metagenome_ID,level=c("KSe054HP",
"KSe055AR",
"KSe063NB",
"KMa157AR",
"KMa159R8",
...
geom.text.size = 3
recruit=ggplot(read_recruit2, aes(x = read_recruit_order, y = level_order, fill = value)) +
    geom_tile(aes(fill = value)) + geom_text(aes(label = round(value, 0)), size=geom.text.size) +
  theme(axis.title=element_text(size=6,face="bold"), legend.text=element_text(size=6), panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(),panel.border = element_rect(color = "black",fill = NA,size = 1), legend.position="none", axis.text.x=element_blank()) + scale_fill_distiller(palette = "Spectral",na.value = "white",) + scale_x_discrete(name="Genome") + scale_y_discrete(name="Metagenome", position = "left") + labs(fill = "Summed\npercent\nrecruitment") +  theme(legend.position = "bottom") +# minimal theme
 theme(axis.text.x = element_text(angle = 45, vjust = 1, 
...
kk+recruit
ggsave('recruit_KByT_Aloha_Jan2025_tree.pdf',width=19, height=14)
ggsave('recruit_KByT_Aloha_Jan2025_tree.svg',width=19, height=14)

```


##Need some abundance stats per Environment for HIMB1412 and Ia.3.V
...
A subset of genes were identified as having distinct distributions with coastal and offshore Pelagibacteraceae. This subset is reported in Supplementary file 2f. The distribution of habitat-specific genes involved in nutrient acquisition and utilization and osmotic and oxidative stress across Pelagibacteraceae genera and serves as the basis of the pN/pS analysis in Figure 5.
...
test=tog_figure5%>%dplyr::arrange(Order)%>%dplyr::select("Category","Metabolism","Specific.function2","Specific.function","FUNCTION","Environment","source_figure4","Present_in_reps","accession","gene_clusters_ids","p_Ia.4.II",	"p_Ia.3.III",	"p_Ia.3.VI",	"p_Ia.3.II",	"p_Ia.3.IV",	"p_Ia.3.V",	"p_Ia.4.1483",	"p_Ia.1.I",	"p_Ia.4.RS39",	"p_Ib.1",	"p_Ia.3.I", "Order")
...
The identified genes above include all the genes in a pathway individually listed row by row. For Figure 4, if genes fall into a single pathway we collaspe them so they take up a single row. Thus we will start this analysis from a different template  
###Figure 4


```{r}
genotype <- read.csv("pathways_to_look_for_Fig_4.csv")
names(genotype)
...
###note that adhesion gene for Ia.3.V was reviewed manually and we determined that the adhesion gene was not present in any Ia.3.V genome. This call is an artifact of the way the enrichment analysis collaspes gene clusters. 
...
###bring in the list of desired genes to examine
set=read.delim("all_other_functions_setup_3.txt")
names(set)
set_KOfam=set%>%subset(source=="KOfam")
set_COG20=set%>%subset(source=="COG20_FUNCTION")
...
tog_figure=tog_figure5%>%dplyr::arrange(Order)%>%dplyr::select("Category","Specific.function","FUNCTION","source","accession","p_Ia.4.II",	"p_Ia.3.III",	"p_Ia.3.VI",	"p_Ia.3.II",	"p_Ia.3.IV",	"p_Ia.3.V",	"p_Ia.4.1483",	"p_Ia.1.I",	"p_Ia.4.RS39",	"p_Ib.1",	"p_Ia.3.I", "Order", "Notes")%>%rename("Lacunapelagibacter"="p_Ia.4.II", "Semelpelagibacter"="p_Ia.4.1483","Proprepelagibacter"="p_Ia.3.IV", "Atlantikopelagibacter"="p_Ia.3.I","Ampluspelagibacter"="p_Ia.3.VI", "Xanthipelagibacter"="p_Ia.3.V", "Coralipelagibacter"="p_Ia.3.II" , "Littoralipelagibacter"="p_Ia.3.III", "Pelagibacter"="p_Ia.1.I", "Superopelagibacter"="p_Ib.1", "Tantilluspelagibacter"="p_Ia.4.RS39", "Specific Function"="Specific.function")
